...
def build_anchor_cluster_convergence_v30(model_sources, top_families=12, audit_top_pairs=None):
    """
    V30.8.4 Anchor Cluster + Pair Assist.

    Idea:
    - 50 nombor 4D beri bahan utama.
    - 2D pair daripada result input / audit_top_pairs beri missing bridge seperti 02 / 20 / 24.
    - Candidate family dibina daripada gabungan nombor model + pair 2D.
    """
    from collections import defaultdict, Counter
    import itertools
    import pandas as pd
    ...
    # pair assist from audit_top_pairs: list like [("02", 12.3), ...]
    pair_assist = []
    try:
        if audit_top_pairs:
            for p in audit_top_pairs:
                ...
                if len(pair) == 2 and pair.isdigit():
                    pair_assist.append(pair)
                    pair_assist.append(pair[::-1])
    except Exception:
        pass

    pair_assist = list(dict.fromkeys(pair_assist))
    ...
    def add_candidate(fam_chars, score, anchor, reason):
        ...
        rec["Family"] = fam
        rec["Score"] += float(score)
        rec["Anchor"].add(anchor)
        if reason not in rec["Reason"]:
            rec["Reason"].append(reason)

    # Rule A: original anchor cluster candidate
    ...
    for anchor, cluster in pair_map.items():
        ...
        if len(extras) >= 2:
            for e1, e2 in itertools.combinations(sorted(extras), 2):
                fam_chars = list(anchor_set) + [e1, e2]
                if len(set(fam_chars)) != 4:
                    continue

                min_extra = min(len(extra_support[e1]), len(extra_support[e2]))
                score = len(cluster) * 3 + min_extra * 18

                add_candidate(
                    fam_chars,
                    score,
                    anchor,
                    f"anchor {anchor} extras {e1}+{e2}"
                )

    # Rule B: Pair Assist.
    # Gabungkan setiap nombor model dengan 2D pair.
    # Contoh:
    # 8054 + 02 => pilih pair 0,2 + char 8,4 => 0248
    # 9044 + 02 => pilih pair 0,2 + char 4,4 => 0244
    for it in items:
        for pair in pair_assist:
            pair_chars = list(pair)

            # choose 2 chars from the model number multiset, allowing double like 44
            for idxs in itertools.combinations(range(4), 2):
                extra_chars = [it["chars"][idxs[0]], it["chars"][idxs[1]]]
                fam_chars = pair_chars + extra_chars

                # allow double family, but require at least 3D overlap with source number or strong pair assist
                fam = "".join(sorted(fam_chars))
                if len(fam) != 4 or fam in exact_families:
                    continue

                overlap = len(set(fam) & it["digits"])
                if overlap < 3:
                    continue

                score = 45 + it["weight"]
                # bonus if pair digits are really present in result pair assist and one digit exists in source
    ...
    for fam, rec in fam_map.items():
        ...
        for it in items:
            ...
            if overlap >= 3:
                support_items.append(it)
                support_score += overlap * 12 + it["weight"]
                sources.add(it["source"])

        support_nos = sorted({it["no"] for it in support_items})
        ...
        rows.append({
            "Family": fam,
            "Score": total_score,
            "Support Count": len(support_nos),
            "Anchor/Pair": ", ".join(sorted(rec["Anchor"])),
            "Support Nos": " / ".join(support_nos),
            "Sources": ", ".join(sorted(sources)),
            "Reason": " | ".join(rec["Reason"][:6]),
        })
