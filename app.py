...
def build_anchor_cluster_convergence_v30(model_sources, top_families=12):
    """
    V30.9 Anchor Cluster Convergence - tuned.
    Uses only current 50 generated numbers.

    Key idea:
    Anchor 2D -> cluster -> extra pair completion.
    Example:
    anchor 08/80 cluster has 8054 and 8052
    extras 4 + 2 should create hidden family 0248.
    """
    from collections import defaultdict, Counter
    import itertools
    import pandas as pd
    ...
    for anchor, cluster in pair_map.items():
        # anchor mesti cukup kuat, tapi jangan terlalu ketat
        if len(cluster) < 2:
            continue

        anchor_set = set(anchor)

        # extra digit frequency outside anchor
        extra_freq = Counter()
        extra_support = defaultdict(set)
        for it in cluster:
            for d in (it["digits"] - anchor_set):
                extra_freq[d] += 1
                extra_support[d].add(it["no"])

        extras = list(extra_freq.keys())
        if len(extras) < 2:
            continue

        # For every pair of extras, create hidden family anchor + extra_pair
        for e1, e2 in itertools.combinations(sorted(extras), 2):
            fam_digits = list(anchor_set) + [e1, e2]
            ...
            for it in cluster:
                ...
                if overlap >= 3:
                    ...
                    # if item has anchor + one of the extras, it supports bridge
                    if anchor_set.issubset(it["digits"]) and (e1 in it["digits"] or e2 in it["digits"]):
                        pair_bridge += 1

            support_nos = {it["no"] for it in support}
            if len(support_nos) < 2:
                continue

            # Balanced extra bonus:
            # Prefer extras that both have support, not just one dominant digit.
            min_extra_support = min(len(extra_support[e1]), len(extra_support[e2]))
            max_extra_support = max(len(extra_support[e1]), len(extra_support[e2]))
            balance_bonus = min_extra_support * 14

            # Pair completion bonus:
            # If there are real numbers in cluster representing both extra sides,
            # such as 8054 and 8052, boost this family heavily.
            pair_completion_bonus = 0
            has_e1 = any(anchor_set.issubset(it["digits"]) and e1 in it["digits"] for it in cluster)
            has_e2 = any(anchor_set.issubset(it["digits"]) and e2 in it["digits"] for it in cluster)
            if has_e1 and has_e2:
                pair_completion_bonus += 35

            # Penalize overly dominant extra pair where one digit dominates too much.
            dominance_penalty = max(0, max_extra_support - min_extra_support - 2) * 6

            # Strong anchor cluster bonus
            cluster_bonus = min(len(cluster), 10) * 3

            final_score = score + balance_bonus + pair_completion_bonus + cluster_bonus - dominance_penalty

            rec = fam_map[fam]
            rec["Family"] = fam
            rec["Score"] += final_score
            rec["Support"].update(support_nos)
            rec["Anchors"].add(anchor)
            reason = f"anchor {anchor}: extras {e1}+{e2}, support {len(support_nos)}"
            if reason not in rec["Reason"]:
                rec["Reason"].append(reason)
    ...
    # Sort by balanced hidden family behavior
