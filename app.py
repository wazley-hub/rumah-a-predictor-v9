...
def build_anchor_cluster_convergence_v30(model_sources, top_families=12):
    """
    V30.8.3 Anchor Cluster Convergence - support fix.

    Perubahan utama:
    - Candidate family masih dibina dari anchor/cluster.
    - Tetapi Support Nos dikira semula daripada SEMUA 50 nombor model.
    - Jadi family 0248 boleh disokong oleh 8054 / 8052 / 8072 / 4852 / 8074 dll,
      bukan terhad kepada support nombor anchor yang sempit.
    """
    from collections import defaultdict, Counter
    import itertools
    import pandas as pd
    ...
    def add_candidate(fam_digits, score, anchor, reason):
        ...
        rec = fam_map[fam]
        rec["Family"] = fam
        rec["Candidate Score"] += float(score)
        rec["Anchor"].add(anchor)
        if reason not in rec["Reason"]:
            rec["Reason"].append(reason)

    # 1) Anchor cluster candidates
    ...
    for anchor, cluster in pair_map.items():
        ...
        if len(extras) >= 2:
            for e1, e2 in itertools.combinations(sorted(extras), 2):
                ...
                max_extra = max(len(extra_support[e1]), len(extra_support[e2]))
                score = len(cluster) * 3 + min_extra * 20 - max(0, max_extra - min_extra - 2) * 5

                # Boost jika memang ada nombor anchor+e1 dan anchor+e2.
    ...
    # 2) Pairwise union candidates.
    # Ini tambahan penting untuk kes 8054 + 8052 -> candidate 0248.
    for a, b in itertools.combinations(items, 2):
        union = sorted(a["digits"] | b["digits"])
        common = a["digits"] & b["digits"]

        if len(union) >= 4 and len(common) >= 2:
            for comb in itertools.combinations(union, 4):
                fam_digits = list(comb)
                score = len(common) * 15 + a["weight"] + b["weight"]
                if a["source"] != b["source"]:
                    score += 8
                add_candidate(
                    fam_digits,
                    score,
                    "".join(sorted(common))[:2] if common else "-",
                    f"pair union {a['no']}+{b['no']}"
                )
    ...
    for fam, rec in fam_map.items():
        ...
        # Recalculate Support Nos globally from all 50 model numbers.
        support_items = []
        support_score = 0.0
        sources = set()
        ...
        # Hidden family mesti ada sekurang-kurangnya 2 support global.
