...
def build_bridge_pair_priority(history, first, second, third):
    """Rank pair semasa ikut berapa kali pair itu diwarisi oleh Top 3 draw berikutnya."""
    columns = ["Priority", "Pair", "Historical Hit", "Historical Use", "Hit Rate %", "Current Source"]
    ...
    for idx in range(len(h) - 1):
        source_pairs = list(dict.fromkeys(
            r["Pair"] for r in _ordered_top3_pairs(
                h.iloc[idx]["first"], h.iloc[idx]["second"], h.iloc[idx]["third"]
            )
        ))
        next_pairs = {
            r["Pair"] for r in _ordered_top3_pairs(
                h.iloc[idx + 1]["first"], h.iloc[idx + 1]["second"], h.iloc[idx + 1]["third"]
            )
        }
    ...
    current_order = list(dict.fromkeys(r["Pair"] for r in current_rows))
    sources = {}
    for row in current_rows:
        label = f'{row["Source"]} {row["Pair Type"]}'
        sources.setdefault(row["Pair"], [])
        if label not in sources[row["Pair"]]:
            sources[row["Pair"]].append(label)

    rows = []
    for original_order, pair in enumerate(current_order):
        used = int(exposure[pair])
        hit = int(hits[pair])
        rows.append({
            "Pair": pair,
            "Historical Hit": hit,
            "Historical Use": used,
            "Hit Rate %": round(hit / used * 100, 1) if used else 0.0,
            "Current Source": " / ".join(sources.get(pair, [])),
            "_Original Order": original_order,
        })

    ranked = pd.DataFrame(rows).sort_values(
        ["Historical Hit", "Hit Rate %", "Historical Use", "_Original Order"],
...
def build_bridge_pair_priority_numbers(pair_priority_df, first, second, third):
    """Kelompok nombor Bridge V1+V2 mengikut kekuatan pair, tanpa family ranking."""
    columns = ["Priority", "Pair", "No", "Route"]
    ...
    for _, ranked_pair in pair_priority_df.iterrows():
        priority = int(ranked_pair["Priority"])
        pair = str(ranked_pair["Pair"])
        seen = set()

        def add_number(d1, d2, route):
            no = f"{pair}{d1}{d2}"
            if no not in seen:
                seen.add(no)
                rows.append({"Priority": priority, "Pair": pair, "No": no, "Route": route})

        for md in missing_digits:
            for ed in existing_digits:
                add_number(md, ed, "V1: Missing + Existing")
        for d1 in missing_digits:
            for d2 in missing_digits:
                if d1 != d2:
                    add_number(d1, d2, "V2: 2 Missing")
        for d1 in existing_digits:
            for d2 in existing_digits:
                if d1 != d2:
                    add_number(d1, d2, "V2: 2 Existing")

    number_df = pd.DataFrame(rows, columns=columns)
    text_lines = ["🧭 Rumah A Predictor - Bridge Pair Priority", ""]
    text_lines.append("Priority Pairs:")
    text_lines.append(" / ".join(
        f'#{int(r["Priority"])} {r["Pair"]}' for _, r in pair_priority_df.iterrows()
    ))
    for priority in pair_priority_df["Priority"].head(3).tolist():
        pair = pair_priority_df.loc[pair_priority_df["Priority"] == priority, "Pair"].iloc[0]
        values = number_df.loc[number_df["Priority"] == priority, "No"].tolist()
        text_lines.extend(["", f"Priority #{int(priority)} — Pair {pair} (Total: {len(values)}):"])
...
def build_combined_family_ranker_v1_v2(v1_df, v2_df, top_n=10):
    ...
    for fam in families:
        ...
        rows.append({
            ...
            "V1 Conviction": str(v1_conv[fam]) if fam in v1_conv else "",
            "V2 Conviction": str(v2_conv[fam]) if fam in v2_conv else "",
            "V1 Balanced": str(v1_bal[fam]) if fam in v1_bal else "",
            "V2 Balanced": str(v2_bal[fam]) if fam in v2_bal else "",
            "Reason": " | ".join(reasons),
        })
    scored_df = pd.DataFrame(rows).sort_values(
        ["Combined Score", "Support", "Family"], ascending=[False, True, True]
    ).reset_index(drop=True)
    # Combined Corrected: V2 mengekalkan Top 3, V1 mengisi slot 4-5,
    # kemudian V2 melengkapkan Top 10. Audit 99 draw = 9/11/13 hit.
    v2_primary = []
    if not v2_df.empty:
    ...
    v1_primary = v1_df.sort_values("Conviction Rank")["Family"].astype(str).map(family4).tolist() if not v1_df.empty else []
    portfolio = list(dict.fromkeys(v2_primary[:3]))
    for fam in v1_primary:
        if fam not in portfolio: portfolio.append(fam)
        if len(portfolio) >= 5: break
    for fam in v2_primary:
        if fam not in portfolio: portfolio.append(fam)
        if len(portfolio) >= int(top_n): break
...
def build_combined_corrected_audit_order_v31_28(v1_df, v2_df, strategy="v2_primary", top_n=10):
    """Bina order Combined alternatif pada saiz shortlist yang sama."""
    def ordered(df, col="Conviction Rank"):
        if df is None or df.empty or "Family" not in df.columns:
            return []
        return df.sort_values(col)["Family"].astype(str).map(family4).drop_duplicates().tolist()
    v1 = ordered(v1_df)
    v2 = ordered(v2_df)
    if strategy == "v2_primary":
        portfolio = list(v2)
    elif strategy == "v1_slot10":
        portfolio, vi, oi = [], 0, 0
        for slot in range(1, int(top_n) + 1):
            pool = v1 if slot == 10 else v2
            idx = oi if slot == 10 else vi
            while idx < len(pool) and pool[idx] in portfolio: idx += 1
            if idx < len(pool): portfolio.append(pool[idx]); idx += 1
            if slot == 10: oi = idx
            else: vi = idx
    elif strategy == "v1_slots5_10":
        portfolio, vi, oi = [], 0, 0
        for slot in range(1, int(top_n) + 1):
            use_v1 = slot in (5, 10)
            pool, idx = (v1, oi) if use_v1 else (v2, vi)
            while idx < len(pool) and pool[idx] in portfolio: idx += 1
            if idx < len(pool): portfolio.append(pool[idx]); idx += 1
            if use_v1: oi = idx
            else: vi = idx
    elif strategy == "v1_slots3_5_10":
        portfolio, vi, oi = [], 0, 0
        for slot in range(1, int(top_n) + 1):
            use_v1 = slot in (3, 5, 10)
            pool, idx = (v1, oi) if use_v1 else (v2, vi)
            while idx < len(pool) and pool[idx] in portfolio: idx += 1
            if idx < len(pool): portfolio.append(pool[idx]); idx += 1
            if use_v1: oi = idx
            else: vi = idx
    elif strategy == "v1_fill_top5":
        portfolio = list(v2[:3])
        for fam in v1:
            if fam not in portfolio: portfolio.append(fam)
            if len(portfolio) >= 5: break
        for fam in v2:
            if fam not in portfolio: portfolio.append(fam)
    else:  # reciprocal rank fusion, V2 masih diberi wajaran lebih besar
        v1r = {f: i + 1 for i, f in enumerate(v1)}
        v2r = {f: i + 1 for i, f in enumerate(v2)}
        families = set(v1r) | set(v2r)
        portfolio = sorted(
            families,
            key=lambda f: (-(100 / (10 + v2r[f]) if f in v2r else 0)
                           -(25 / (10 + v1r[f]) if f in v1r else 0), f),
        )
    for fam in v2 + v1:
        if fam not in portfolio: portfolio.append(fam)
    return portfolio
...
def run_backtest_bridge_dde_lite_v31_24_5(history_df, test_draws=30):
    ...
    for idx in range(start_idx, latest_idx + 1):
        try:
            ...
            v2_unique_map = {f: i + 1 for i, f in enumerate(v2_unique_list)}
            combined_audit_maps = {
                strategy: {f: i + 1 for i, f in enumerate(
                    build_combined_corrected_audit_order_v31_28(v1_rank_df, v2_rank_df, strategy, top_n=10)
                )}
                for strategy in (
                    "v2_primary", "v1_slot10", "v1_slots5_10", "v1_slots3_5_10",
                    "v1_fill_top5", "rrf",
                )
            }
            ...
            if has_next:
                ...
                combined_audit_hits = {}
                for strategy, rank_map in combined_audit_maps.items():
                    ranks = [rank_map[f] for f in actual_fams if f in rank_map]
                    combined_audit_hits[strategy] = tuple(
                        "YES" if ranks and min(ranks) <= cutoff else "NO" for cutoff in (3, 5, 10)
                    )
            ...
            row_result = {
                ...
                **{
                    f"Combined Audit {strategy} Top{cutoff} Hit": combined_audit_hits[strategy][pos]
                    for strategy in combined_audit_hits
    ...
    combined_audit_totals = {
        (strategy, cutoff): int(valid.get(
            f"Combined Audit {strategy} Top{cutoff} Hit", pd.Series(dtype=str)
        ).eq("YES").sum())
        for strategy in (
            "v2_primary", "v1_slot10", "v1_slots5_10", "v1_slots3_5_10",
            "v1_fill_top5", "rrf",
...
"""
    ...
    # -----------------------------
    st.subheader("🧭 Bridge Pair Priority")
    st.caption(
        "Susun pair semasa mengikut jumlah kejayaan sejarah pair yang sama muncul semula "
        "dalam Top 3 draw berikutnya. Nombor Bridge V1 + V2 kemudian dikelompokkan mengikut priority pair."
    )
    try:
        ...
        if pair_priority_df.empty:
            st.info("Data sejarah belum mencukupi untuk Bridge Pair Priority.")
        else:
            priority_pairs = " / ".join(
                f'#{int(row["Priority"])} {row["Pair"]}'
            ...
            with st.expander("Lihat nombor mengikut Priority Pair", expanded=False):
                selected_priority = st.selectbox(
                    ...
                    format_func=lambda value: (
                        f'Priority #{value} — Pair '
                        f'{pair_priority_df.loc[pair_priority_df["Priority"] == value, "Pair"].iloc[0]}'
                    ),
                    key="bridge_pair_priority_selector_v31_35",
                )
                selected_numbers = pair_priority_numbers_df[
                    pair_priority_numbers_df["Priority"] == selected_priority
                ].copy()
                st.caption(f"Jumlah nombor untuk priority ini: {len(selected_numbers)}")
                st.dataframe(selected_numbers, hide_index=True, use_container_width=True)
    except Exception as e:
        st.warning(f"Bridge Pair Priority belum dapat dipaparkan: {e}")
