...
def build_cross_month_top10(history, first, second, third, lookback_events=10):
    """Eksperimen berasingan: 5 Route + 5 Slot daripada hit lintas bulan."""
    if history is None or len(history) < 3:
        return {"numbers": [], "route": "", "slot": ""}

    frame = history.reset_index(drop=True).copy()
    frame["_date"] = pd.to_datetime(
        frame["draw_date"].astype(str).str.replace(r"\.0$", "", regex=True),
        format="%Y%m%d",
        errors="coerce",
    )
    learned = []
    for target_index in range(1, len(frame)):
        source_index = target_index - 1
        source_date = frame.iloc[source_index]["_date"]
        target_date = frame.iloc[target_index]["_date"]
        if pd.isna(source_date) or pd.isna(target_date):
            continue
        if source_date.month == target_date.month:
            continue
        source = [
            _pad4(frame.iloc[source_index][col])
            for col in ("first", "second", "third")
        ]
        targets = {
            _key4(frame.iloc[target_index][col])
            for col in ("first", "second", "third")
        }
        winners = [
            row for row in _candidates(source) if row["key"] in targets
        ]
        routes, slots = set(), set()
        for winner in winners:
            routes.update(winner["routes"])
            slots.update(winner["slots"])
        if routes or slots:
            learned.append({"routes": routes, "slots": slots})
            learned = learned[-int(lookback_events):]

    def favourite(field):
        counts, last_seen = Counter(), {}
        for sequence, event in enumerate(learned):
            for value in event[field]:
                counts[value] += 1
                last_seen[value] = sequence
        if not counts:
            return ""
        return sorted(
            counts,
            key=lambda value: (-counts[value], -last_seen[value], value),
        )[0]

    fav_route, fav_slot = favourite("routes"), favourite("slots")
    current = _candidates([_pad4(first), _pad4(second), _pad4(third)])
    route_rows = [
        row for row in current if fav_route and fav_route in row["routes"]
    ]
    slot_rows = [
        row for row in current if fav_slot and fav_slot in row["slots"]
    ]
    selected, known = [], set()
    ...
    add(slot_rows[:5])
    if len(selected) < 10:
        for index in range(max(len(route_rows), len(slot_rows))):
            add(route_rows[index:index + 1])
            add(slot_rows[index:index + 1])
            if len(selected) >= 10:
                break
    return {
        "numbers": [row["No"] for row in selected],
        "route": fav_route,
        "slot": fav_slot,
    }


def next_regular_draw_date(value):
    current = pd.to_datetime(
        str(value).replace(".0", ""), format="%Y%m%d", errors="coerce"
    )
    if pd.isna(current):
        return None
    for days in range(1, 8):
        candidate = current + pd.Timedelta(days=days)
        if candidate.dayofweek in (2, 5, 6):
            return candidate
    return None
