...
def build_anchor_cluster_convergence_v30(model_sources, top_families=12):
    """
    V30.8.1 Anchor Cluster Convergence - safe tune.
    Uses only current 50 generated numbers.

    Tuned to value extra-pair completion:
    8054 + 8052 under anchor 08/80 should help create hidden family 0248.
    """
    from collections import defaultdict, Counter
    import itertools
    import pandas as pd
    ...
    for source, nums in model_sources:
        for rank, n in enumerate(nums, start=1):
            ...
            items.append({
                ...
                "digits": set(s),
                "rank": rank,
                "weight": source_weight.get(source, 2) + max(0, 11 - min(rank, 10)) * 0.2,
            })
    ...
    for anchor, cluster in pair_map.items():
        ...
        for e1, e2 in itertools.combinations(sorted(extras), 2):
            ...
            # Balanced extra pair bonus.
            # Extra digits that both appear in the anchor cluster should be boosted.
            min_extra = min(len(extra_support[e1]), len(extra_support[e2]))
            max_extra = max(len(extra_support[e1]), len(extra_support[e2]))
            balance_bonus = min_extra * 18

            # Pair completion bonus:
            # anchor + e1 exists and anchor + e2 exists in separate support numbers.
            ...
            pair_bonus = 35 if has_e1 and has_e2 else 0

            # Reduce overly dominant single digit bias.
            dominance_penalty = max(0, max_extra - min_extra - 2) * 8

            cluster_bonus = min(len(cluster), 10) * 3
            source_bonus = len({it["source"] for it in support}) * 6

            final_score = score + balance_bonus + pair_bonus + cluster_bonus + source_bonus - dominance_penalty

            rec = fam_map[fam]
            rec["Family"] = fam
            rec["Anchors"].add(anchor)
            rec["Support"].update(support_nos)
            rec["Clusters"].update([it["no"] for it in cluster])
            rec["Score"] += final_score

            reason = f"anchor {anchor}, extra {e1}+{e2}, support {len(support_nos)}"
            if reason not in rec["Reason"]:
                rec["Reason"].append(reason)
